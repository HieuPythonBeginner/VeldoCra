...
Stmt* Parser::parse_simple_statement() {
    ...
    // Memory statements
    if (check(TokenType::Kw_Forge) || check(TokenType::Kw_Pur) || check(TokenType::Kw_Ee)) {
        return parse_memory_statement();
    }
    ...
    while (true) {
        if (check(TokenType::ParenOpen)) {
            ...
            if (check(TokenType::Identifier)) {
                ...
                advance();
                expr = builder_.create_member(expr, member, false);
            }
            ...
            expr = builder_.create_index(expr, index);
        } else {
            break;
        }
    }
...
Expr* Parser::parse_primary() {
    ...
    if (check(TokenType::Identifier) || check(TokenType::Kw_Scribe)) {
        std::string_view name_view = get_token_text(current_token());
        const char* name = builder_.alloc_string(name_view);
        advance();
...
LiteralExprNode* Parser::parse_literal() {
    ...
    switch (token.type) {
        ...
        case TokenType::String: {
            ...
            // Pass string_view directly so arena copy gets correct length
            return builder_.create_literal(text);
        }
...
Expr* Parser::parse_type_expr() {
    ...
    if (check(TokenType::Identifier)) {
        ...
        if (check(TokenType::Lt)) {
            ...
            // Create type expression with parameters
            std::vector<Expr*> params;
            if (first_param) params.push_back(first_param);
            if (second_param) params.push_back(second_param);
            return builder_.create_type_expr(type_name, params);
        }

        // Simple type: int, str, bool, float, etc.
        return builder_.create_type_expr(type_name, {});
    }
...
/**
 * @brief Parse memory management statement (forge, pur, ee)
 */
Stmt* Parser::parse_memory_statement() {
    if (check(TokenType::Kw_Forge)) {
        advance(); // consume forge
        Expr* size_expr = parse_expression();
        if (check(TokenType::Semicolon)) advance();
        // forge size -> alloc(size)
        CallExprNode* call = builder_.create_call(builder_.create_variable("alloc"));
        builder_.add_argument(call, size_expr);
        return builder_.create_expr_stmt(call);
    } else if (check(TokenType::Kw_Pur)) {
        advance(); // consume pur
        Expr* num_expr = parse_expression();
        Expr* size_expr = parse_expression();
        if (check(TokenType::Semicolon)) advance();
        // pur num size -> calloc(num, size)
        CallExprNode* call = builder_.create_call(builder_.create_variable("calloc"));
        builder_.add_argument(call, num_expr);
        builder_.add_argument(call, size_expr);
        return builder_.create_expr_stmt(call);
    } else if (check(TokenType::Kw_Ee)) {
        advance(); // consume ee
        Expr* ptr_expr = parse_expression();
        if (check(TokenType::Semicolon)) advance();
        // ee ptr -> free(ptr)
        CallExprNode* call = builder_.create_call(builder_.create_variable("free"));
        builder_.add_argument(call, ptr_expr);
        return builder_.create_expr_stmt(call);
    }

    report_error("Expected forge, pur, or ee");
    synchronize();
    return builder_.create_expr_stmt(builder_.create_none());
}

/**
 * @brief Parse OOP definition (race/class, avatar/struct) - stub
 */
Stmt* Parser::parse_oop_definition() {
    // TODO: Implement proper OOP parsing
    ...
    report_error("OOP definitions not implemented yet");
    synchronize();
    return builder_.create_expr_stmt(builder_.create_none());
}
