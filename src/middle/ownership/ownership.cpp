...
OwnershipInfo* OwnershipContext::lookup(std::string_view name) {
    // ASM/BOOTSTRAP mode: skip undefined for lexer.veldan keywords and builtins
...
void OwnershipChecker::walk_node(ast::Node* node) {
    ...
    std::cerr << "[Ownership] walk_node: kind=" << (int)node->kind << ", ptr=" << node << "\n";
...
void OwnershipChecker::handle_fn(ast::FnStmtNode* fn_stmt) {
    ...
    // Declare parameters with null pointer and bounds checking
    std::cerr << "[HANDLE_FN] param_count=" << fn_stmt->param_count << std::endl;
    
    // Safety check: ensure params array is not null before iterating
    if (fn_stmt->params && fn_stmt->param_count > 0) {
        // Add reasonable upper bound to prevent potential issues
        const size_t MAX_REASONABLE_PARAMS = 100;
        size_t safe_param_count = (fn_stmt->param_count < MAX_REASONABLE_PARAMS) 
                                   ? fn_stmt->param_count 
                                   : MAX_REASONABLE_PARAMS;
        
        for (size_t i = 0; i < safe_param_count; ++i) {
            // Check each param pointer before accessing
            if (fn_stmt->params[i] && fn_stmt->params[i]->name) {
    ...
    if (fn_stmt->body) {
        walk_node(static_cast<ast::Node*>(fn_stmt->body));
    }
...
void OwnershipChecker::handle_variable(ast::VariableExprNode* var_expr) {
    std::cerr << "[Ownership] handle_variable called, var_expr=" << var_expr << "\n";
    if (!var_expr || !var_expr->name) {
        std::cerr << "[Ownership] var_expr null or name null\n";
        return;
    }

    std::cerr << "[Ownership] var_expr name ptr: " << (void*)var_expr->name << "\n";

    size_t name_len = strlen(var_expr->name);
    std::cerr << "[Ownership] name length: " << name_len << "\n";
    std::cerr << "[Ownership] name content: '";
    for (size_t i = 0; i < name_len; ++i) {
        std::cerr << var_expr->name[i];
    }
    std::cerr << "'\n";

    std::string var_name(var_expr->name);

    std::cerr << "[Ownership] var_name: '" << var_name << "'\n";
    ...
    if (builtins.count(var_name)) {
        std::cerr << "[Ownership] Builtin found, skipping\n";
        return;
    }
...
void OwnershipChecker::handle_call(ast::CallExprNode* call_expr) {
    ...
    std::cerr << "[Ownership] handle_call: callee kind=" << (int)call_expr->callee->kind << "\n";
