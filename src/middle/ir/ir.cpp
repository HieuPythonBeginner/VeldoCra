...
IRInstruction* IRBuilder::create_binary(IROpcode op, IRValue* left, IRValue* right, Type* result_type) {
    IRInstruction* inst = new IRInstruction(op, result_type, current_block_);
...
IRInstruction* IRBuilder::create_gep(IRValue* base, IRValue* index, Type* result_type) {
    auto* inst = new IRInstruction(IROpcode::GEP, result_type, current_block_);
...
IRGenerator::IRGenerator() : module_(nullptr), builder_(nullptr) {}

IRGenerator::~IRGenerator() {
    delete builder_;
}

IRModule* IRGenerator::generate(veldanava::ast::Node* root) {
    module_ = new IRModule();
    builder_ = new IRBuilder(module_);

    // Create main function
    Type* void_type = Type::get_void();
    IRFunction* main_func = builder_->create_function("main", void_type);
    IRBasicBlock* entry = builder_->create_block(main_func, "entry");
    builder_->set_current_block(entry);

    // Visit root
    if (root->kind == veldanava::ast::NodeKind::ExprStmt) {
        veldanava::ast::ExprStmtNode* stmt = static_cast<veldanava::ast::ExprStmtNode*>(root);
        visit(stmt->expr);
    }
...
IRValue* IRGenerator::visit(veldanava::ast::Node* node) {
    switch (node->kind) {
        case veldanava::ast::NodeKind::LiteralExpr:
            return visit_literal(static_cast<veldanava::ast::LiteralExprNode*>(node));
        case veldanava::ast::NodeKind::VariableExpr:
            return visit_variable(static_cast<veldanava::ast::VariableExprNode*>(node));
        case veldanava::ast::NodeKind::BinaryExpr:
            return visit_binary(static_cast<veldanava::ast::BinaryExprNode*>(node));
        case veldanava::ast::NodeKind::CallExpr:
            return visit_call(static_cast<veldanava::ast::CallExprNode*>(node));
        default:
            return nullptr;
    }
}

IRValue* IRGenerator::visit_literal(veldanava::ast::LiteralExprNode* node) {
    if (node->literal_type == 2) { // string
        return builder_->get_string(std::string(node->value.string_val));
    } else if (node->literal_type == 0) { // int
        return builder_->get_int32(std::stoi(std::string(node->value.string_val)));
    }
    return nullptr;
}

IRValue* IRGenerator::visit_variable(veldanava::ast::VariableExprNode* node) {
    // TODO: lookup variable
    return nullptr;
}

IRValue* IRGenerator::visit_binary(veldanava::ast::BinaryExprNode* node) {
    IRValue* left = visit(node->left);
    IRValue* right = visit(node->right);
    IROpcode op = IROpcode::INVALID;
    switch (node->op) {
        case static_cast<uint16_t>(veldanava::lexer::TokenType::Plus): op = IROpcode::ADD; break;
        case static_cast<uint16_t>(veldanava::lexer::TokenType::Minus): op = IROpcode::SUB; break;
        case static_cast<uint16_t>(veldanava::lexer::TokenType::Star): op = IROpcode::MUL; break;
        case static_cast<uint16_t>(veldanava::lexer::TokenType::Slash): op = IROpcode::DIV; break;
        // Add more as needed
        default: op = IROpcode::ADD; break; // fallback
    }
    return builder_->create_binary(op, left, right, builder_->get_int32_type());
}

IRValue* IRGenerator::visit_call(veldanava::ast::CallExprNode* node) {
    // For scribe, create PRINT
    if (std::string(static_cast<veldanava::ast::VariableExprNode*>(node->callee)->name) == "scribe") {
        IRValue* arg = visit(node->args[0]);
        builder_->create_call(nullptr, {arg}, builder_->get_void_type());
        return nullptr;
    }
    return nullptr;
}
